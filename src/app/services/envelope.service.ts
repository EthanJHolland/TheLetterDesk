...
export class EnvelopeService {
    ...
    private envelopeUrl = 'http://localhost:3000';  // Url that all Envelope 's will be stored at
    
    constructor(private http: Http) { }

    getEnvelope(_id: string): Promise<Envelope> {  //pulls off a envelope class object envelope from a storage at a url
        const url = `${this.envelopeUrl}/sender/${_id}`;  //creates the url that will be accessed, based on the id given
    ...
    update(envelope: Envelope): Promise<Envelope> {   //puts a Envelope class object envelope at the url envelopeUrl/letter.id
        const url = `${this.envelopeUrl}`;//${envelope._id}`;   //generates the storage url for envelope
